...
@plt_settings()
def plot_results(file="path/to/results_duibi.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    """
    Plot training results_duibi from a results_duibi CSV file. The function supports various types of data including segmentation,
    pose estimation, and classification. Plots are saved as 'results_duibi.png' in the directory where the CSV is located.

    Args:
        file (str, optional): Path to the CSV file containing the training results_duibi. Defaults to 'path/to/results_duibi.csv'.
    ...
    Example:
        ...
        plot_results('path/to/results_duibi.csv', segment=True)
        ```
    """
    ...
    ax = ax.ravel()
    files = list(save_dir.glob("results_duibi*.csv"))
    assert len(files), f"No results_duibi.csv files found in {save_dir.resolve()}, nothing to plot."
    for f in files:
        try:
            data = pd.read_csv(f)
            ...
            for i, j in enumerate(index):
                ...
                ax[i].plot(x, y, marker=".", label=f.stem, linewidth=2, markersize=8)  # actual results_duibi
    ...
    ax[1].legend()
    fname = save_dir / "results_duibi.png"
...
def plot_tune_results(csv_file="tune_results.csv"):
    """
    Plot the evolution results_duibi stored in an 'tune_results.csv' file. The function generates a scatter plot for each key
    in the CSV, color-coded based on fitness scores. The best-performing configurations are highlighted on the plots.

    Args:
        csv_file (str, optional): Path to the CSV file containing the tuning results_duibi. Defaults to 'tune_results.csv'.
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    Args:
        ...
        save_dir (Path, optional): Directory to save results_duibi. Defaults to Path('runs/detect/exp').
    """
