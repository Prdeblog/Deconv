...
def run_sql_query():
    """Executes an SQL query and returns the results_duibi."""
...
def run_ai_query():
    """Execute SQL query and update session state with query results_duibi."""
    ...
    if query.rstrip().lstrip():
        ...
        if not isinstance(res, pandas.DataFrame) or res.empty:
            st.session_state["error"] = "No results_duibi found using AI generated query. Try another query or rerun it."
            return
